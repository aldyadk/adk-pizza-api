      ...
      if (err) {
        ...
        if (hashedPassword) {
          ...
          _data.create("users", hashedEmail, userData, function(err) {
            if (!err) {
              const cartObject = { id: hashedEmail };
              _data.create("carts", hashedEmail, cartObject, function(err) {
                if (!err) {
                  cb(200);
                } else {
                  cb(500, { Error: "Could not create cart" });
                }
              });
            } else {
              console.log(err);
...
_users.get = function(data, cb) {
  ...
  if (email) {
    ...
    _users.verifyToken(token, email, function(tokenIsValid) {
      if (tokenIsValid) {
        const hashedEmail = helpers.hash(email);
        _data.read("users", hashedEmail, function(err, userData) {
          if (!err && userData) {
...
_users.verifyToken = function(id, email, cb) {
